...
class TouhouAutoPlayer:
    ...
    def move_player(self, player, enemy_bullets, enemy_lasers, enemies, boss_eyes):
        ...
        smooth = self.panic_smooth if current_danger > 25000 else self.normal_smooth
        ...
        player.rect.x += int(round(self.vx))
        player.rect.y += int(round(self.vy))
...
class Map3:
    ...
    def draw_score(self):
        text = self.font.render(f"SCORE: {self.score // 10}", True, WHITE)
    ...
    def game_over(self):
        ...
        while True:
            ...
            help_text = self.font.render("CLICK / R: RESTART    ESC: MENU", True, WHITE)
